import { Buffer } from 'buffer-es6' // Required for aws sigv4 signing
import * as url from '../../../url'
import * as browserCreateHash from 'aws-sdk/lib/browserCryptoLib'

window.Buffer = window.Buffer || Buffer

var encrypt = function (key, src, encoding) {
  if (encoding === void 0) { encoding = '' }
  return browserCreateHash.default.createHmac('sha256', key).update(src, 'utf8').digest(encoding)
}
var hash = function (src) {
  src = src || ''
  return browserCreateHash.default.createHash('sha256').update(src, 'utf8').digest('hex')
}
/**
* @private
...
var parse_service_info = function (request) {
  var url_info = url.parse(request.url); var host = url_info.host
  var matched = host.match(/([^.]+)\.(?:([^.]*)\.)?amazonaws\.com$/); var parsed = (matched || []).slice(1, 3)
...
var get_signing_key = function (secret_key, d_str, service_info) {
  ...
  var k = ('AWS4' + secret_key); var k_date = encrypt(k, d_str); var k_region = encrypt(k_date, service_info.region); var k_service = encrypt(k_region, service_info.service); var k_signing = encrypt(k_service, 'aws4_request')
  return k_signing
}
...
var sign = function (request, access_info, service_info) {
  ...
  var dt = new Date(); var dt_str = dt.toISOString().replace(/[:-]|\.\d{3}/g, ''); var d_str = dt_str.substr(0, 8); var algorithm = 'AWS4-HMAC-SHA256'
  var url_info = url.parse(request.url)
  ...
  var signing_key = get_signing_key(access_info.secret_key, d_str, service_info); var signature = get_signature(signing_key, str_to_sign)
