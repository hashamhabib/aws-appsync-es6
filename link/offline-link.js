...
import * as constants_1 from '@redux-offline/redux-offline/lib/constants';
...
import * as apollo_client_1 from 'apollo-client';
// var retry_link_1 = require("./retry-link");
import * as retry_link_1 from './retry-link';
...
export function isOptimistic(obj) { return typeof obj[IS_OPTIMISTIC_KEY] !== undefined ? obj[IS_OPTIMISTIC_KEY] : undefined; };
            ...
            if (isMutation) {
                ...
                if (!doIt) {
                    ...
                    if (enqueuedMutations === 0) {
                        boundSaveSnapshot(_this.store, cache);
                    }
...
export { OfflineLink };
export function boundSaveSnapshot(store, cache) { return store.dispatch(saveSnapshot(cache)); };
        ...
        return __generator(this, function (_d) {
            switch (_d.label) {
                ...
                case 1:
                    ...
                    variables = __assign({}, replaceUsingMap(__assign({}, origVars), idsMap), (_a = {}, _a[retry_link_1.SKIP_RETRY_KEY] = true, _a));
                    optimisticResponse = replaceUsingMap(__assign({}, origOptimistic), idsMap);
                    return [2 /*return*/, new Promise(function (resolve, reject) {
                        ...
                        console.log('Executing link', operation);
                        apollo_link_1.execute(client.link, operation).subscribe({
                                ...
                                enquededMutations
                                    ...
                                    .forEach(function (_a) {
                                        ...
                                        if (typeof update !== 'function') {
                                            console.log('No update function for mutation', { document: document, variables: variables });
                                            return;
                                        }
                                        var optimisticResponse = replaceUsingMap(__assign({}, origOptimisticResponse), idsMap);
                                        var result = { data: optimisticResponse };
                                        if (fetchPolicy !== 'no-cache') {
                                            console.log('Running update function for mutation', { document: document, variables: variables });
                                            dataStore.markMutationResult({
                                                mutationId: null,
                                                result: result,
                                                document: document,
                            ...
                            error: function (err) {
                                console.log('Error when executing link', err);
                                reject(err);
                            }
                        });
                    })];
...
var _discard = function (error, action, retries) {
    ...
    if (graphQLErrors.length) {
        console.log('Discarding action.', action, graphQLErrors);
        return true;
    }
    else {
        ...
        if (appSyncClientError) {
            console.log('Discarding action.', action, appSyncClientError);
            return true;
        }
    }
...
//#region utils
export function replaceUsingMap(obj, map) {
    if (map === void 0) { map = {}; }
    if (!obj || !map) {
        return obj;
    }
    ...
    if (typeof obj === 'object') {
        Object.keys(obj).forEach(function (key) {
            ...
            if (Array.isArray(val)) {
                obj[key] = val.map(function (v, i) { return replaceUsingMap(v, map); });
            }
...
export function getIds(dataIdFromObject, obj, path, acc) {
    if (path === void 0) { path = ''; }
    if (acc === void 0) { acc = {}; }
    if (!obj) {
        return acc;
